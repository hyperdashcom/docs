...
<Note>
  A stop limit trades price risk for fill risk: size the gap between stop and limit for the market's volatility so a fast move cannot jump both. When execution matters more than price, use a [Stop Market](/hyperdash/orders/stop-market).
</Note>
