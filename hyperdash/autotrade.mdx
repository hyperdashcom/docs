...
Autotrade is the execution engine behind [copy trading](/hyperdash/copy-trade) on Hyperdash. It mirrors (Copytrade) or takes the opposite side of (Countertrade) the perp positions of up to 10 target traders, trading automatically from a wallet dedicated to that strategy.

## How it works

<Steps>
  <Step title="Choose your targets">
    Start from a trader profile, the Copytraders section of [Explore](/hyperdash/wallet-explore), or paste any wallet address. One Autotrade wallet can follow up to 10 traders, with capital split equally or by custom weights.
  </Step>
  ...
  <Step title="Start and manage">
    Press **Start**. The wallet shows as Running on the Copytrade tab with its equity, PNL, positions, and logs. Open **Autotrade Settings** any time to edit configuration, pause, or resume.
  </Step>
</Steps>
...
| Only copy newly opened positions | Skips positions already open when Autotrade starts; changeable only while stopped | Off |
...
- **Delete** permanently removes an autotrade wallet from your list.
...
<AccordionGroup>
  <Accordion title="What does Autotrade cost?">
  ...
  <Accordion title="Can I change traders after starting?">
    Yes, from the Allocation tab in Autotrade Settings. Removing a trader closes its copied positions on the next sync.
  </Accordion>
</AccordionGroup>
